...
```
index.html      → o app inteiro: markup + CSS + JS inline (parser, renderer, tamanho, wake lock, registro do SW)
...
|---------|----------------|-----------|
| `VOICES` | ~126 | Mapa `A`/`B`/`AB` → `{ name, cls }`. **Único lugar** para renomear os cantores. `cls` liga o código à cor CSS (`--voice-${cls}`, `.t-${cls}`, `.r-${cls}`) |
| `parse(md)` | ~140 | Markdown → `{ title, sections[] }`. Uma passada linha a linha; ver "Regras do parser" abaixo |
| `renderPitch(text)` | ~167 | Troca `^` por `<span class="pitch">↑</span>` e `_` por `↓`, só quando colados numa palavra (`/\^(?=\S)/`) |
| `lineHtml(ln, isCounter)` | ~172 | Uma linha → `<div class="line">` com fita lateral (`.ribbon`) + texto. Resolve a etiqueta "2ª: Nome" do `[AB>X]` e aplica a classe `counter-text` no contra-canto |
| `render(i)` | ~180 | Monta e injeta a música `i` em `#stage`; persiste o índice |
| `boot()` | ~198 | Carrega tudo; em qualquer erro mostra o aviso de `file://` em `.notice` |
| `size` / `apply()` | ~217 | Tamanho da letra: grava `--lyric-size` no `:root` e persiste. Botões `+`/`−` andam de 3px, clamp **22–72px**, padrão 40px |
| `keepAwake()` | ~228 | `navigator.wakeLock.request('screen')`; re-adquire em `visibilitychange`. Falha silenciosa onde não há suporte |
| registro do SW | ~239 | `navigator.serviceWorker.register('sw.js')` no evento `load` |
...
- Nome do cache: `dueto-v1` (constante `CACHE`).
